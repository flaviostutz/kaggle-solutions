...
def dataset_xy_balance_classes_image(input_h5file_path, output_h5file_path, max_augmentation_ratio=3, max_undersampling_ratio=1, classes_weight=1, image_data_generator=None):
    ...
    with input_h5:
        input_x_ds = input_h5['X'][50:80]#FIXME remove later
        input_y_ds = input_h5['Y'][50:80]#FIXME remove later
        if(len(input_x_ds)==0):
        ...
        qtty_per_class = min(smallest_qtty*(1-max_augmentation_ratio), largest_qtty)
        ...
        ratio_classes = classes_weight * ratio_classes

        logger.info('creating output dataset ' + output_h5file_path)
        output_h5 = h5py.File(output_h5file_path, 'w')
        with output_h5:
            output_x_ds, output_y_ds = create_xy_dataset(output_h5, x_dims, y_dims)
            ...
            for i,x in enumerate(input_x_ds):
                ...
                #augmentation
                    ...
                    #accept sample
                    add_sample_to_dataset(output_x_ds, output_y_ds, x, y)
                    pending_augmentations[label] = pending_augmentations[label] + r
